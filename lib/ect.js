		...
		var
			...
			read = function (file) {
				if (Object.prototype.toString.call(ect.options.root) === '[object Object]') {
					...
					// file is an array too!
					var goodFile = file.reduce(function(prev, curr) {
						if (prev) { return prev; }
					...
					if (goodFile) {
						return fs.readFileSync(goodFile, 'utf8');
					} else {
						throw new Error ('Failed to load template ' + file.join(','));
					}
				} else {
					try {
			...
			if (ect.options.cache && cache[template]) {
				...
				if (Object.prototype.toString.call(ect.options.root) === '[object String]') {
					if (typeof process !== 'undefined' && process.platform === 'win32') {
						file = path.normalize(
							(ect.options.root.length && template.charAt(0) !== '/' && template.charAt(0) !== '\\' && !/^[a-zA-Z]:/.test(template) ? (ect.options.root + '/') : '') + template.replace(extExp, '') + ect.options.ext
						);
					} else {
					...
					for (var i = 0; i < ect.options.root.length; i++) {
						var myFile;
						var myRoot = ect.options.root[i];
						if (typeof process !== 'undefined' && process.platform === 'win32') {
							myFile = path.normalize(
								(myRoot.length && template.charAt(0) !== '/' && template.charAt(0) !== '\\' && !/^[a-zA-Z]:/.test(template) ? (myRoot + '/') : '') + template.replace(extExp, '') + ect.options.ext
							);
						} else {
							myFile = path.normalize((myRoot.length && template.charAt(0) !== '/' ? (myRoot + '/') : '') + template.replace(extExp, '') + ect.options.ext);
						}
						file.push(myFile);
					}
				} else {
